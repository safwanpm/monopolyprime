...
const ContactSignature = () => {
  ...
  // Optimized Step Handler (Prevents stale state issues)
  const nextStep = (category, value) => {
    setSelections((prev) => ({ ...prev, [category]: value }));
    setActiveStep((prev) => (prev < steps.length ? prev + 1 : prev));
  };

  // Memoized WhatsApp Link (Prevents recreation every render)
...
Please connect me with a specialist.`;
  ...
  return (
    <>
      ...
      <section
        className="relative pt-24 bg-[#050505] min-h-screen min-h-[100svh] text-white flex flex-col justify-between overflow-hidden"
      ...
      >
              ...
              {activeStep < steps.length ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {steps[activeStep].options.map((opt) => (
                      <button
                        key={opt}
                        onClick={() =>
                          nextStep(steps[activeStep].id, opt)
                        }
                        className="group relative p-8 bg-white/[0.03] border border-white/10 rounded-2xl text-left hover:bg-white hover:text-black transition-all duration-500 transform-gpu"
                      >
                        <Plus
                          size={16}
                          className="absolute top-6 right-6 text-primary group-hover:text-black transition-colors"
                        />
                ...
                >
                  ...
                  <p className="text-black/70 text-sm font-medium mb-10 max-w-[280px] mx-auto">
                    Message sent will include your choices for{" "}
                  ...
                  <Link
                    href={whatsappLink}
                    target="_blank"
                    className="inline-flex items-center gap-4 bg-black text-white px-10 py-5 rounded-full font-black uppercase text-[10px] tracking-[0.2em] hover:scale-105 transition-transform transform-gpu"
                  >
                  ...
                  <button
                    onClick={() => {
                      setActiveStep(0);
                      setSelections({ goal: "", region: "", timeline: "" });
                    }}
        ...
        <div className="absolute top-1/2 left-0 -translate-y-1/2 w-[600px] h-[600px] bg-primary/5 blur-[80px] md:blur-[120px] rounded-full pointer-events-none" />
      </section>
